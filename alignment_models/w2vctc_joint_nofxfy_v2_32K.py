...
# =========================
print("Step 0: Config")
exp_name = "w2vCTC_joint_v2_32K"
model_name    = "results/w2vCTC/checkpoint-23430"
output_dir    = f"results/{exp_name}"
vocab_name    = "vocab_w2vCTC-woSIL.json"
preprocessed_path = "/vol/experiments3/imbenamor/TAPAS-FRAIS/alignment_models/charsiu-main/results/preprocessed32k"
...
if os.path.exists(preprocessed_path):
    ...
    _feature_extractor = Wav2Vec2FeatureExtractor(
        feature_size=1, sampling_rate=TARGET_SR,
    ...
    def preprocess_batch(batch):
        ...
        assert all(sr == TARGET_SR for sr in sampling_rates)
...
print(f"Min length: {min(lengths)/TARGET_SR:.1f}s")
print(f"Max length: {max(lengths)/TARGET_SR:.1f}s")
...
# Loss weights
# CHANGES:
#   λ_ctc : 1.0 → 3.0  (protect the pretrained encoder from being overwritten)
#   λ_fs  : 0.5 → 1.0  (stronger monotonicity signal to fight attractor formation)
#   λ_m   : 0.5         (unchanged — still ramped slowly for quantizer stability)
# =========================
def get_loss_weights(global_step: int,
    ...
    # λ_fs=0.15 brings weighted L_fs (~7*0.15=1.05) in line with weighted L_ctc (~1.05)
    # Adjust based on what you observe in train/weighted_* after first 500 steps
...
class Wav2Vec2ForCTC_FS_REC(nn.Module):
    ...
    def compute_phoneme_embeddings_from_ctc(
        ...
        for b in range(B):
            ...
            for p_id in labels_clean[b]:
                ...
                if weight_sum < 1e-6:   # phoneme has no posterior mass — fallback to uniform
                    weights = torch.ones_like(weights) / weights.shape[0]
                else:
                    weights = weights / weight_sum
                emb = (weights.unsqueeze(-1) * X[b]).sum(0)# (T,)
    ...
    def compute_alignment(self, X, Y_emb):
        ...
        n_idx = torch.arange(N, device=X.device).float() / max(N - 1, 1)  # (N,)
        t_idx = torch.arange(T, device=X.device).float() / max(T - 1, 1)  # (T,)
    
        diff = n_idx.unsqueeze(1) - t_idx.unsqueeze(0)  # (N, T): positive = above diagonal
    
        # Symmetric Gaussian — penalizes being off diagonal in either direction
        gaussian = -10.0 * diff ** 2
    
        # One-sided penalty — extra cost for being AHEAD of diagonal (n/N > t/T)
        # This prevents the path from racing through phonemes too fast
        ahead_penalty = -20.0 * torch.clamp(diff, min=0) ** 2
    ...
    def forward(self, input_values, attention_mask=None, labels=None, global_step=0):
        ...
        # In forward(), before compute_alignment:
...
class FSCTCTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"  ✗ NaN/Inf loss at step {self.state.global_step} — skipping update")
            # Return zero loss to skip this step without crashing
            loss = loss * 0.0
    ...
    def _log_loss_components(self, loss, loss_ctc, loss_fs, loss_m, inputs, outputs):
        ...
        print(msg)
        # In _log_loss_components, always log these regardless of step:
        print(f"  A_min={outputs['alignment'].min().item():.2e}  A_max={outputs['alignment'].max().item():.2e}")
        self.log({
    ...
    def _compute_alignment_metrics(self, A, labels_clean):
        """
        A:            (B, N, T)
        labels_clean: (B, N)
        Returns a dict of scalar metrics averaged over the batch.
        """
        B, N, T = A.shape
        metrics = {
            "monotonicity":      [],
        ...
        for b in range(B):
            n_phones = (labels_clean[b] != 0).sum().item()
            if n_phones == 0:
                continue
    
            A_b    = A[b, :n_phones, :]          # (N_real, T)
            path   = A_b.argmax(dim=0)           # (T,) — which phoneme owns each frame
    
            # 1. Monotonicity: fraction of frame transitions that go forward
            transitions  = path[1:] - path[:-1]  # (T-1,)
            forward      = (transitions >= 0).float().mean().item()
            metrics["monotonicity"].append(forward)
    
            # 2. Coverage: fraction of phonemes assigned at least one frame
            assigned     = path.unique().numel()
            coverage     = assigned / n_phones
            metrics["coverage"].append(coverage)
    
            # 3. Dominant phoneme ratio: frames taken by the single busiest phoneme
            counts       = torch.bincount(path, minlength=n_phones)
            dominant     = counts.max().item() / T
            metrics["dominant_ratio"].append(dominant)
    
            # 4. Diagonal score: correlation of argmax path with ideal diagonal
            ideal        = torch.linspace(0, n_phones - 1, T, device=A.device)
            path_f       = path.float()
            # Normalize both to [0,1] then compute mean absolute deviation
            path_norm    = path_f / max(n_phones - 1, 1)
            ideal_norm   = ideal / max(n_phones - 1, 1)
            diag_score   = 1.0 - (path_norm - ideal_norm).abs().mean().item()
            metrics["diagonal_score"].append(diag_score)
    
            # 5. Per-frame max weight: how peaked each column of A is
            per_frame_max = A_b.max(dim=0).values.mean().item()
            metrics["per_frame_max"].append(per_frame_max)
    
        return {k: float(np.mean(v)) if v else 0.0 for k, v in metrics.items()}
    # ------------------------------------------------------------------
    # Optimizer — UPDATED: removed fx, fy, phoneme_embed param groups
    # ------------------------------------------------------------------
    def create_optimizer(self):
        fe_params = list(self.model.wav2vec2.feature_extractor.parameters())
        fe_ids    = {id(p) for p in fe_params}
        enc_rest  = [p for p in self.model.wav2vec2.parameters() if id(p) not in fe_ids]
    
        self.optimizer = torch.optim.AdamW([
            {"params": fe_params,                                              "lr": 5e-5},
            {"params": enc_rest,                                               "lr": 1e-5},
            {"params": list(self.model.ctc_head.parameters()),                 "lr": 1e-5},
    ...
    def _log_alignment_sharpness(self):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Entropy-based sharpness (existing)
                eps       = 1e-8
                ...
                # NEW: detailed alignment metrics
                align_metrics = self._compute_alignment_metrics(A, labels_clean)
    
            print(
                f"\n[alignment @ step {self._step_count}]\n"
                f"  sharpness:      {sharpness:.1f}%\n"
                f"  monotonicity:   {align_metrics['monotonicity']:.3f}   (1.0 = perfect)\n"
                f"  coverage:       {align_metrics['coverage']:.3f}       (1.0 = all phones used)\n"
                f"  dominant_ratio: {align_metrics['dominant_ratio']:.3f} (low = no attractor)\n"
                f"  diagonal_score: {align_metrics['diagonal_score']:.3f} (1.0 = perfect diagonal)\n"
                f"  per_frame_max:  {align_metrics['per_frame_max']:.3f}  (high = peaked columns)\n"
            )
            self.log({
                "alignment_sharpness":          sharpness,
                "alignment/monotonicity":       align_metrics["monotonicity"],
                "alignment/coverage":           align_metrics["coverage"],
                "alignment/dominant_ratio":     align_metrics["dominant_ratio"],
                "alignment/diagonal_score":     align_metrics["diagonal_score"],
                "alignment/per_frame_max":      align_metrics["per_frame_max"],
            })
    
        except Exception as e:
            print(f"Alignment metrics logging failed: {e}")
        finally:
            model.train()
