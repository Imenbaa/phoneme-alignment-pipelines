...
sorted_indices = np.argsort(lengths)  # ascending — shortest first
...
def get_loss_weights(global_step: int,
                     ...
                     warmup_m: int = 5000):
    """
    L_ctc active from step 1 — keeps encoder anchored to phoneme recognition.
    L_fs ramps over 800 steps — starts shaping alignment quickly.
    L_m ramps over 5000 steps — gives quantizer time to stabilize first.
    """
    w_fs = min(1.0, global_step / max(warmup_fs, 1))
    w_m = min(1.0, global_step / max(warmup_m, 1))
    return 1.0, 1.0 * w_fs, 1.0 * w_m
...
base_model = Wav2Vec2ForCTC.from_pretrained(
    model_name,  # pretrained CTC checkpoint
...
# ── Initialize phoneme embedding from CTC head weights ────────────
print("Initializing phoneme embedding from CTC head weights...")
with torch.no_grad():
    model.phoneme_embed.weight.data.copy_(model.ctc_head.weight.data)
print(f"Done — shape: {model.phoneme_embed.weight.shape}")
# ─────────────────────────────────────────────────────────────────
...
training_args = TrainingArguments(
    ...
    warmup_steps=200,
    weight_decay=0.005,
    learning_rate=1e-4,  # 10× lower than Stage 1
    num_train_epochs=20,
...
class FSCTCTrainer(Trainer):
    ...
    def create_optimizer(self):
        self.optimizer = torch.optim.AdamW([
            {
                # Pretrained — small lr to preserve recognition quality
                "params": list(self.model.wav2vec2.parameters()),
                "lr": 1e-5,
            },
            {
                # Pretrained CTC head — small lr
                "params": self.model.ctc_head.parameters(),
                "lr": 1e-5,
            },
            {
                # Random init alignment modules — need to learn fast
            ...
            {
                # Random init reconstruction head
                "params": self.model.reconstruction_head.parameters(),
                "lr": 1e-4,
            },
            {
                # Random init quantizer projection — highest lr
...
# After model instantiation — before curriculum loop


trainer = FSCTCTrainer(
    model=model,
    data_collator=data_collator,
    args=training_args,  # normal num_train_epochs=15
    compute_metrics=compute_metrics,
    train_dataset=sorted_dataset,  # sorted by length
