...
for checkpoint_path in checkpoints:
    if args.exp_name=="w2vCTC_VAD_SIL":
    ...
    for i in range(0, len(test_data), batch_size):
        ...
        if args.exp_name=="w2vCTC_VAD_SIL":
            vad = [torch.tensor(v) for v in batch["vad"]]
    ...
    for i in range(min(5, len(all_pred_tokens))):
        ...
        print()
    if args.exp_name=="w2vCTC_VAD_SIL":
        def remove_sil(tokens):
            return [t for t in tokens if t != "SIL"]

        per = np.mean([
            editdistance.eval(remove_sil(p), remove_sil(l)) / max(1, len(remove_sil(l)))
