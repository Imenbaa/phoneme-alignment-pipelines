...
# =========================
print("Step 0: Config")
exp_name = "w2vCTC_FS_stage2_curriculum"
...
# Curriculum training
print("Creating curriculum buckets...")

def get_duration_seconds(example):
    return example["length"] / 16000   # length is in samples at 16kHz

# Define buckets by duration
short_dataset  = dataset["train"].filter(
    lambda x: x["length"] < 48000,      # < 3 seconds
    num_proc=4
)
medium_dataset = dataset["train"].filter(
    lambda x: 48000 <= x["length"] < 96000,   # 3–6 seconds
    num_proc=4
)
long_dataset   = dataset["train"].filter(
    lambda x: x["length"] >= 96000,     # > 6 seconds
    num_proc=4
)

print(f"Short:  {len(short_dataset)} samples")
print(f"Medium: {len(medium_dataset)} samples")
print(f"Long:   {len(long_dataset)} samples")
###########################################
# =========================
...
def compute_metrics(pred):
    ...
    for pred_id_seq, label_id_seq in zip(pred_ids, label_ids):
        ...
        label_tokens = [processor.tokenizer.convert_ids_to_tokens(int(i)) for i in label_id_seq if
                        i not in [-100, processor.tokenizer.pad_token_id]]
...
def initialize_quantizer(model, dataloader, device, n_batches=200):
    ...
    # Initialize both codevectors and ema_embed buffers
...
class GumbelQuantizerEMA(nn.Module):
    """
    Gumbel softmax quantizer with Exponential Moving Average codebook update.
    EMA stabilizes codebook entries much faster than gradient descent —
    each entry tracks the running average of frames assigned to it,
    producing consistent targets q_t from early in training.
    """
    def __init__(self, input_dim: int, num_vars: int = 320,
        ...
        # Projection: maps frame (B, T, D) → codebook logits (B, T, num_vars)
        self.weight_proj = nn.Linear(input_dim, num_vars)

        # Codebook stored as buffers — updated by EMA, not by gradient
    ...
    def forward(self, X: torch.Tensor):
        B, T, D = X.shape
        logits   = self.weight_proj(X)              # (B, T, num_vars)

        if self.training:
            probs = F.gumbel_softmax(logits, tau=self.temp, hard=True)  # (B, T, num_vars)

            # EMA update — no gradient through codebook
            with torch.no_grad():
                flat_probs = probs.reshape(-1, self.num_vars)   # (B*T, num_vars)
                flat_X     = X.reshape(-1, D)                   # (B*T, D)

                # Count assignments per codebook entry
                counts    = flat_probs.sum(0)                   # (num_vars,)
                embed_sum = flat_probs.T @ flat_X               # (num_vars, D)

                # EMA update of cluster sizes and embeddings
                self.cluster_size = (self.decay * self.cluster_size
                                     + (1 - self.decay) * counts)
                self.ema_embed    = (self.decay * self.ema_embed
                                     + (1 - self.decay) * embed_sum)

                # Normalize to get updated codebook entries
                self.codevectors  = (self.ema_embed
                                     / self.cluster_size.unsqueeze(1).clamp(min=1e-5))
        else:
            indices = logits.argmax(dim=-1)                     # (B, T)
            probs   = F.one_hot(indices, self.num_vars).float() # (B, T, num_vars)

        quantized = torch.matmul(probs, self.codevectors)       # (B, T, D)
...
class Wav2Vec2ForCTC_FS_REC(nn.Module):
    ...
    def __init__(
        ...
        # Alignment module
        self.phoneme_embed = nn.Embedding(vocab_size, hidden_dim)
        self.fx = nn.Linear(hidden_dim, hidden_dim)
        self.fy = nn.Linear(hidden_dim, hidden_dim)
    ...
    def mask_features(self, X: torch.Tensor, p_low: float = 0.1, p_high: float = 0.5):
        """
        Variable masking probability sampled per batch from [p_low, p_high].
        Forces the reconstruction head to always rely on YA regardless of
        how many frames are masked.
        """
        B, T, _ = X.shape
    ...
    def forward_sum_loss(self, A, label_lengths, input_lengths):
        """
        Forward-sum loss via the CTC trick from the paper.
    ...
    def forward(self, input_values, attention_mask=None, labels=None, global_step=0):
        ...
        loss_ctc = F.ctc_loss(logits.log_softmax(dim=-1).transpose(0, 1), labels, input_lengths,
                              (labels != -100).sum(dim=-1), blank=0, reduction="mean", zero_infinity=True, )
        ...
        self.quantizer.temp = current_temp          # controls Gumbel sharpness
...
# Initialize new EMA buffers from loaded codevectors
with torch.no_grad():
    model.quantizer.ema_embed.copy_(model.quantizer.codevectors.clone())
    model.quantizer.cluster_size.fill_(1.0)
print("EMA buffers initialized from Stage 1 codevectors")
# ─────────────────────────────────────────────────────────────────
...
# ── initialize_quantizer call goes here ──────────────────────────
    ...
    def create_optimizer(self):
        self.optimizer = torch.optim.AdamW([
            ...
            },
            {
                ...
                "lr": 1e-3,   # only weight_proj has gradient now
            },
        ], weight_decay=self.args.weight_decay)
        return self.optimizer
...
# Initialize quantizer on short sequences only — cleaner signal
...
# Curriculum training
curriculum = [
    ("short",  short_dataset,  4),
    ("medium", medium_dataset, 3),
    ("long",   long_dataset,   3),
]

last_checkpoint = None
current_epoch = 0
for stage_name, bucket, n_epochs in curriculum:
    stage_output = f"{output_dir}/curriculum_{stage_name}"

    # ── Skip if already completed ──────────────────────────────────
    if os.path.exists(os.path.join(stage_output, "trainer_state.json")):
        print(f"Stage {stage_name} already completed — skipping")
        last_checkpoint = stage_output
        current_epoch  += n_epochs
        continue
    # ──────────────────────────────────────────────────────────────

    print(f"\n=== Curriculum: {stage_name} ({len(bucket)} samples, {n_epochs} epochs) ===")
    os.makedirs(stage_output, exist_ok=True)

    stage_args = replace(
        training_args,
        num_train_epochs = current_epoch + n_epochs,
        output_dir       = stage_output,
        logging_dir      = f"{stage_output}/tensorboard",
    )
    ...
    last_checkpoint = stage_output
    print(f"Stage {stage_name} complete. Checkpoint: {stage_output}")
# Save final logs from last trainer instance
