...
# Step 0: Config
# CHANGES:
#   RESUME_STEP = 14841  (offset for λ warmup — resumes from correct λ values)
#   checkpoint_dir       (latest checkpoint to resume from)
# =========================
print("Step 0: Config")

RESUME_STEP   = 14841                                          # ← CHANGED: checkpoint step
checkpoint_dir = "results/w2vCTC_bertphone/checkpoint-14841"  # ← CHANGED: resume checkpoint

exp_name          = "w2vCTC_bertphone"
...
# Step 9: Model
# CHANGES:
#   align_temperature: 15.0 (unchanged, already good)
#   ahead_penalty:     -40.0 (was -20.0 — doubled to fix running-ahead)
# =========================
...
class Wav2Vec2ForCTC_FS_REC(nn.Module):
    ...
    def __init__(
        ...
        self.align_temperature = 15.0   # unchanged
    ...
    def to(self, *args, **kwargs):
        """Keep phoneme_bert always on GPU with rest of model."""
    ...
    def decode_to_ipa(self, labels_clean: torch.Tensor) -> list:
        ...
        for seq in labels_clean:
            ...
            for token_id in seq:
                ...
                if token_id == 0:
                    break
                corpus_label = self.ctc_tokenizer.convert_ids_to_tokens(token_id)
    ...
    def compute_phoneme_embeddings(self, labels_clean, device):
        ...
        H_bert  = bert_out.last_hidden_state[:, 1:-1, :]
    ...
    def compute_alignment(self, X: torch.Tensor, Y_emb: torch.Tensor) -> torch.Tensor:
        ...
        ahead_penalty = -40.0 * torch.clamp(diff, min=0) ** 2   # ← CHANGED: was -20
...
# =========================
# Load base model + resume from checkpoint
# =========================
...
# Load checkpoint weights — overwrites all base_model weights
sft_path = os.path.join(checkpoint_dir, "model.safetensors")
...
# Both should be empty — confirms full checkpoint loaded

# =========================
# Step 10: Trainer
# CHANGES:
#   warmup_steps = 0         (no warmup needed when resuming)
#   metric_for_best_model = "PER"   (was alignment_sharpness)
...
training_args = TrainingArguments(
    ...
    warmup_steps                = 0,             # ← CHANGED: no warmup when resuming
    weight_decay                = 0.005,
    learning_rate               = 1e-4,
    num_train_epochs            = 30,
    metric_for_best_model       = "PER",         # ← CHANGED: was alignment_sharpness
    greater_is_better           = False,         # ← CHANGED: lower PER is better
    logging_steps               = 1000,
...
class FSCTCTrainer(Trainer):
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        msg = (
            ...
            f"L_ctc={mean_ctc:.4f}  L_fs={mean_fs:.4f}  L_m={mean_m:.4f}"
        )
        print(msg)
    ...
    def _log_loss_components(self, loss, loss_ctc, loss_fs, loss_m, inputs, outputs):
        λ_ctc, λ_fs, λ_m = get_loss_weights(self.state.global_step + RESUME_STEP)
        with torch.no_grad():
...
# Launch
# CHANGES:
#   NO initialize_quantizer  — weights loaded from checkpoint
#   NO initialize_bert_proj  — weights loaded from checkpoint
#   trainer.train()          — starts from step 0 counter but uses RESUME_STEP offset
# =========================
...
# ── NO initialize_quantizer — checkpoint weights already trained ──────
# ── NO initialize_bert_proj — checkpoint weights already trained ──────
