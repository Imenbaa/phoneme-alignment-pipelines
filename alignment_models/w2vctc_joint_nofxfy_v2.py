...
# =========================
print("Step 0: Config")
exp_name = "w2vCTC_joint_v2_softpool"
...
def get_loss_weights(global_step: int, warmup_fs: int = 800, warmup_m: int = 5000):
    ...
    # Stronger alignment signal than the original code, but still warmed up.
...
class GumbelQuantizerEMA(nn.Module):
    def __init__(self, input_dim: int, num_vars: int = 320, temp: float = 2.0, decay: float = 0.99):
        super().__init__()
...
class Wav2Vec2ForCTC_FS_REC(nn.Module):
    """
    Wav2Vec2 + forward-sum alignment + soft posterior pooling + contrastive reconstruction.

    Main changes vs the original code:
      1) Greedy hard labels are kept only to define the predicted phoneme order.
      2) Phoneme embeddings are built from soft posteriors, not detached hard pooling.
      3) A trainable fy is added on the phoneme side.
      4) Gradients can flow from L_fs / L_m back into logits and encoder through soft pooling.
    """
    ...
    # ------------------------------------------------------------------
    # Soft posterior pooling over the predicted phoneme order
    # ------------------------------------------------------------------
    def compute_phoneme_embeddings_from_ctc(
        self,
        X: torch.Tensor,           # (B, T, D)
        logits: torch.Tensor,      # (B, T, vocab)
        labels_clean: torch.Tensor # (B, N) hard decoded phoneme IDs, used only for ordering
    ) -> torch.Tensor:
        """
        Differentiable pooling.

        Step 1: build a soft token bank from the full posterior
                token_bank[b, v, d] = sum_t P(v|t) * X[b,t,d] / sum_t P(v|t)

        Step 2: use the greedy-decoded predicted phoneme order only to select
                which token embeddings to use in the sequence.

        Gradients flow through the posterior weights back into logits and X.
        The hard decoded IDs do NOT carry gradients, but the pooled embeddings do.
        """
        B, T, D = X.shape
        ctc_probs = logits.softmax(dim=-1)  # (B, T, V)

        # soft token-level embeddings
        token_mass = ctc_probs.sum(dim=1).clamp(min=1e-6)            # (B, V)
        token_bank = torch.einsum("btd,btv->bvd", X, ctc_probs)      # (B, V, D)
        token_bank = token_bank / token_mass.unsqueeze(-1)           # normalize

        Y_emb_list = []
        for b in range(B):
            seq = labels_clean[b]
            seq = seq[seq != 0]  # remove PAD/blank (id 0)
            if seq.numel() == 0:
                seq = torch.tensor([1], device=X.device, dtype=torch.long)

            # gather predicted token embeddings in the predicted order
            phone_embs = token_bank[b].index_select(0, seq)
            Y_emb_list.append(phone_embs)
    ...
    # ------------------------------------------------------------------
    # Alignment with direct dot-product + diagonal prior
    # ------------------------------------------------------------------
    def compute_alignment(self, X, Y_emb):
        ...
        gaussian = -30.0 * diff ** 2
        ahead_penalty = -40.0 * torch.clamp(diff, min=0) ** 2
    ...
    def forward(self, input_values, attention_mask=None, labels=None, global_step=0):
        ...
        # Greedy CTC decode — no gradients needed for the order extraction itself.
        with torch.no_grad():
            labels_clean = self._ctc_decode_batch(logits)

        # Soft posterior pooling: gradients can flow from Y_emb back to logits and X.
...
class FSCTCTrainer(Trainer):
    ...
    Custom trainer with:
      ...
      - Per-epoch averaged loss logging
      - Posterior sharpness monitoring
    ...
    def create_optimizer(self):
        # Soft-pooling makes the CTC branch part of the alignment loop,
        # so the encoder / CTC head need a real but still conservative LR.
        self.optimizer = torch.optim.AdamW([
            {"params": list(self.model.wav2vec2.parameters()), "lr": 5e-6},
            {"params": list(self.model.ctc_head.parameters()), "lr": 5e-6},
            ...
            {"params": list(self.model.quantizer.weight_proj.parameters()), "lr": 1e-4},
