...
def vad_chunk_with_timestamps_v3(
    ...
    return chunks
def vad_chunk_with_timestamps_v2(
    wav,
    sampling_rate=16000,
    max_chunk_duration=10.0,
    max_pause_duration=1.5,
    ...
    wav: torch.Tensor (1D, 16kHz)
    returns: list of dicts with:
        start/end      → true speech boundaries
        pad_start/end  → padded boundaries used for audio extraction
    """
    vad_model, utils = torch.hub.load(
        repo_or_dir="snakers4/silero-vad",
        model="silero_vad",
    ...
    for seg in split_ts:
        ...
        pause = seg_start - chunk_end
        if pause <= max_pause_duration and (seg_end - chunk_start) <= max_chunk_duration:
            chunk_end = seg_end
        else:
            chunks.append({"start": chunk_start, "end": chunk_end})
            chunk_start = seg_start
