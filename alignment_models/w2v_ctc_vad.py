...
def compute_energy_vad(audio, sr=16000, frame_ms=20, threshold=0.005):
    ...
    return np.array(vad, dtype=np.float32) # output is a vector or 0 and 1 of length = number of frames (1value is 20ms)
...
if os.path.exists(preprocessed_path):
    ...
    def preprocess_batch(batch):
        ...
        with processor.as_target_processor():
            labels = processor(phonemes).input_ids

        vad = [compute_energy_vad(a) for a in audio_arrays]
    ...
    for split in ["train", "validation", "test"]:
        dataset[split] = dataset[split].map(
            ...
            cache_file_name=f"/vol/experiments/cache_imbenamor/map_vadnosil_{split}.arrow"
        )
