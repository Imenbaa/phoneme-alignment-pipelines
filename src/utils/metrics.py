...
def match_alignments_lev(ref_alignments, hyp_alignments, ref_seq, hyp_seq,
    ...
    aas = (
        (sum(start_errors) + sum(end_errors))
        / (2 * K)
        if K > 0 else np.nan
    )

    return start_errors, end_errors, duration_errors, mid_errors, aas
...
def metrics_rhap(alignment_store,csv_path):
    ...
    for f, data in alignment_store.items():
        ...
        # Boundary errors
        start_err, end_err, dur_err, mid_err,aas = \
        ...
        results.append({
            ...
            "style":                   style_val,
            "K": len(start_err),
            "AAS_numerator": (sum(start_err) + sum(end_err)),
            ...
            "F1@50ms (%)":             f1_50 * 100,
            "F1@0ms (%)":  f1_0 * 100,
            "AAS (ms)": aas * 1000 if not np.isnan(aas) else np.nan,
    ...
    def aggregate_group(group_df, label, style_val):
        total_num = group_df["AAS_numerator"].sum()
        ...
        return {
            ...
            "AAS (ms)": (total_num / (2 * total_K) * 1000 if total_K > 0 else np.nan),
